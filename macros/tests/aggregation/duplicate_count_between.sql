{% test duplicate_count_between(model, columns, min_count, max_count, group_by=None, where=None) %}
...
select
    ...
    duplicate_row_count as actual_duplicate_count,
    {{ min_count }} as expected_min_count,
    {{ max_count }} as expected_max_count,
    evaluated_row_count,
    unique_row_count,
    duplicate_row_count,
    'duplicate_count_between' as failed_check,
    'Duplicate count for {{ columns | join(", ") }} must be between {{ min_count }} and {{ max_count }}' as failure_reason,
    {{ dbt_checks.applied_condition(where) }} as applied_condition

from validation

where
    {{ dbt_checks.build_between_predicate('duplicate_row_count', min_count, max_count) }}
