{% test distinct_ratio_between(model, column_name, min_ratio, max_ratio, group_by=None, where=None) %}
...
with
{{ dbt_checks.calculate_distinct_ratio_cte(
    model,
    column_name,
    where,
    group_by
...
select
    ...
    'distinct_ratio_between' as failed_check,
    'Distinct ratio for {{ column_name }} must be between {{ min_ratio }} and {{ max_ratio }}' as failure_reason,
